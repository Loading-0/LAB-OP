PROGRAM InsertionSortB(INPUT, OUTPUT);{Сортирует символы из INPUT}
CONST
  Max = 16;
  ListEnd = 0;
TYPE
...
BEGIN {InsertionSort}
  ...
  DO
    BEGIN                                                                    
      ...
      ELSE
        BEGIN
          READ(Arr[Index].Key) {Включение Arr[Index] в связанный список}
        END
    END; {WHILE}
  
  { обработать последний уже прочитанный символ }
  Prev := 0;
  Curr := First;
  Found := FALSE;
  WHILE (Curr <> 0) AND NOT Found 
