PROGRAM SortDate(INPUT, OUTPUT);

TYPE
  Month = (NoMonth, Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec);
  DayNum = 1..31;
  Date = RECORD
    Mo  : Month;
    Day : DayNum;
  END;
  FileOfDate = FILE OF Date;

VAR
  Copying: BOOLEAN;
  D, VarDate: Date;
  TFile, DateFile: FileOfDate;
  FInput: TEXT;

{ --- ВСЕ ПРОЦЕДУРЫ И ФУНКЦИИ ВНУТРИ ОДНОГО ФАЙЛА --- }

PROCEDURE ReadMonth(VAR FIn: TEXT; VAR Mo: Month);
VAR
  Ch1, Ch2, Ch3: CHAR;
BEGIN {ReadMonth}
  READ(FIn, Ch1, Ch2, Ch3);
  IF (Ch1 = 'J') AND (Ch2 = 'A') AND (Ch3 = 'N') THEN Mo := Jan ELSE
  IF (Ch1 = 'F') AND (Ch2 = 'E') AND (Ch3 = 'B') THEN Mo := Feb ELSE
  IF (Ch1 = 'M') AND (Ch2 = 'A') AND (Ch3 = 'R') THEN Mo := Mar ELSE
  IF (Ch1 = 'A') AND (Ch2 = 'P') AND (Ch3 = 'R') THEN Mo := Apr ELSE
  IF (Ch1 = 'M') AND (Ch2 = 'A') AND (Ch3 = 'Y') THEN Mo := May ELSE
  IF (Ch1 = 'J') AND (Ch2 = 'U') AND (Ch3 = 'N') THEN Mo := Jun ELSE
  IF (Ch1 = 'J') AND (Ch2 = 'U') AND (Ch3 = 'L') THEN Mo := Jul ELSE
  IF (Ch1 = 'A') AND (Ch2 = 'U') AND (Ch3 = 'G') THEN Mo := Aug ELSE
  IF (Ch1 = 'S') AND (Ch2 = 'E') AND (Ch3 = 'P') THEN Mo := Sep ELSE
  IF (Ch1 = 'O') AND (Ch2 = 'C') AND (Ch3 = 'T') THEN Mo := Oct ELSE
  IF (Ch1 = 'N') AND (Ch2 = 'O') AND (Ch3 = 'V') THEN Mo := Nov ELSE
  IF (Ch1 = 'D') AND (Ch2 = 'E') AND (Ch3 = 'C') THEN Mo := Dec
  ELSE Mo := NoMonth;
END; {ReadMonth}

PROCEDURE WriteMonth(VAR FOut: TEXT; VAR Mo: Month);
BEGIN {WriteMonth}
  IF Mo = Jan THEN WRITE(FOut, 'JAN') ELSE
  IF Mo = Feb THEN WRITE(FOut, 'FEB') ELSE
  IF Mo = Mar THEN WRITE(FOut, 'MAR') ELSE
  IF Mo = Apr THEN WRITE(FOut, 'APR') ELSE
  IF Mo = May THEN WRITE(FOut, 'MAY') ELSE
  IF Mo = Jun THEN WRITE(FOut, 'JUN') ELSE
  IF Mo = Jul THEN WRITE(FOut, 'JUL') ELSE
  IF Mo = Aug THEN WRITE(FOut, 'AUG') ELSE
  IF Mo = Sep THEN WRITE(FOut, 'SEP') ELSE
  IF Mo = Oct THEN WRITE(FOut, 'OCT') ELSE
  IF Mo = Nov THEN WRITE(FOut, 'NOV') ELSE
  IF Mo = Dec THEN WRITE(FOut, 'DEC')
  ELSE WRITE(FOut, 'NoMonth');
END; {WriteMonth}

PROCEDURE ReadDate(VAR FIn: TEXT; VAR Res: Date);
BEGIN {ReadDate}
  ReadMonth(FIn, Res.Mo);
  READ(FIn, Res.Day);
END; {ReadDate}

PROCEDURE WriteDate(VAR FOut: TEXT; VAR Res: Date);
BEGIN {WriteDate}
  WriteMonth(FOut, Res.Mo);
  WRITE(FOut, Res.Day:3);
END; {WriteDate}

FUNCTION Less(VAR D1, D2: Date): BOOLEAN;
BEGIN {Less}
  IF D1.Mo < D2.Mo 
  THEN 
    Less := TRUE
  ELSE IF D1.Mo > D2.Mo 
  THEN
    Less := FALSE
  ELSE Less := (D1.Day < D2.Day);
END; {Less}

PROCEDURE CopyOut(VAR DateFile: FileOfDate);
VAR
  VarDate: Date;
BEGIN {CopyOut}
  WHILE NOT EOF(DateFile) 
  DO
  BEGIN
    READ(DateFile, VarDate);
    WriteDate(OUTPUT, VarDate);
    WRITELN;
  END;
END; {CopyOut}

{ --- ГЛАВНЫЙ БЛОК ПРОГРАММЫ --- }
BEGIN {SortDate}
  ASSIGN(DateFile, 'DF.DAT');
  ASSIGN(TFile, 'TF.DAT');
  ASSIGN(FInput, 'FI.TXT');
  
  REWRITE(DateFile);
  RESET(FInput);
  
  { Считываем самую первую дату для начального заполнения файла }
  ReadDate(FInput, VarDate);
  READLN(FInput);
  WRITE(DateFile, VarDate);
  
  { Основной цикл обработки последующих дат из FInput }
  RESET(DateFile);
  WHILE NOT EOF(FInput) 
  DO
  BEGIN
    { --- Раздел DP 1.1 --- }
    ReadDate(FInput, D);
    READLN(FInput);
    
    IF (D.Mo <> NoMonth) 
    THEN
    BEGIN
      { --- Раздел DP 1.1.1 (Копируем элементы, меньшие чем D) --- }
      REWRITE(TFile);
      RESET(DateFile);
      Copying := TRUE;
      
      WHILE NOT EOF(DateFile) AND Copying 
      DO
      BEGIN
        READ(DateFile, VarDate);
        IF Less(VarDate, D) 
        THEN
          WRITE(TFile, VarDate)
        ELSE
          Copying := FALSE;
      END;
      { --- Конец раздела DP 1.1.1 --- }
      
      { Копируем саму вставляемую дату D в TFile }
      WRITE(TFile, D);
      
      { Если цикл прервался из-за элемента >= D, возвращаем его на место }
      IF NOT Copying 
      THEN
        WRITE(TFile, VarDate);
        
      { Дописываем все оставшиеся элементы из DateFile в TFile }
      WHILE NOT EOF(DateFile) 
      DO
      BEGIN
        READ(DateFile, VarDate);
        WRITE(TFile, VarDate);
      END;
      
      { Переносим отсортированные данные обратно в DateFile }
      RESET(TFile);
      REWRITE(DateFile);
      WHILE NOT EOF(TFile) 
      DO
        BEGIN
          READ(TFile, VarDate);
          WRITE(DateFile, VarDate);
        END;
    END;
    { --- Конец раздела DP 1.1 --- }
  END;

  { --- Раздел DP 1.2 (Вывод DateFile в OUTPUT) --- }
  RESET(DateFile);
  CopyOut(DateFile);
  
  { Освобождение файловых дескрипторов }
  CLOSE(FInput);
  CLOSE(DateFile);
  CLOSE(TFile);
END. {SortDate}

